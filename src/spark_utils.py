...
def build_stack_expression(cols):
...
def unpivot(logger:Logger, df:DataFrame, columns_to_keep:list, dimension_column_name:str, value_column_name:str) -> DataFrame:
    ...
    stack = build_stack_expression(columns)
...
def get_dataframe(logger:Logger, spark:SparkSession, file_path:str):
...
def get_spark_context(app_name=__name__):
